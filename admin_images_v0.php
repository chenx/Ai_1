...
<!--
Max Upload file size is <?= ($max_file_size) / 1024 ?> KB. <br/>
-->

<input type='hidden' id="win_h" name="win_h" value="<?=(isset($_REQUEST['win_h']) ? $_REQUEST['win_h'] : $win_h)?>"/>
<input type="hidden" name="MAX_FILE_SIZE" value="<?=$max_file_size?>"> <!--max size: in Byte-->

<span title='Current gallery'>Choose gallery: 
<?=getDirsAsSelect($dir_default);?>
</span>
&nbsp;&nbsp;&nbsp;
...
<input type='text' id='newDir' name='newDir' value="<?=$newDir?>" size='10' />
</span>
&nbsp;&nbsp;&nbsp;
...
<input type='checkbox' name='doOverwrite' value='Y' <?= ($doOverwrite ? 'checked' : '') ?> style='vertical-align: middle;' />Overwrite</span>
</span>
&nbsp;&nbsp;&nbsp;

<input type='submit' name='btnLoop' value='Slide Show' title='Slide show all images' /> 
<?= getLoopIntervalList($doLoopInt); ?>sec 
<span title='Show popup window'>
<input type='checkbox' name='doPopup' value='Y' <?= ($doPopup ? 'checked' : '') ?> style='vertical-align: middle;' />Popup</span>
...
<div style="width:100%;height:<?=$win_h?>px;overflow:auto; padding: 5px; border-color: #cccccc; border-width: 1px; ">
